...
<div class="flex h-screen overflow-hidden">
    ...
    <div id="sidebar"
        ...
        {{-- Sidebar Top --}}
        <div class="shrink-0 flex items-center h-14 px-4 border-b border-sidebar-2-divider overflow-hidden">
            <a href="{{ route('dashboard') }}" class="shrink-0 flex items-center justify-center size-7 rounded-md bg-primary">
                <svg class="size-4 fill-primary-foreground" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/>
                </svg>
            </a>
            <span class="sidebar-label ms-2.5 font-semibold text-sm text-foreground whitespace-nowrap overflow-hidden transition-all duration-300">Aspirasi Pro</span>
        </div>

        {{-- Sidebar Body --}}
        <nav class="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-6">
            ...
            <div class="sidebar-label overflow-hidden transition-all duration-300">
                <button type="button"
                    ...
                    <span class="ms-auto flex items-center gap-x-1 py-px px-1.5 border border-line-2 rounded text-[10px] text-muted-foreground-1">⌘K</span>
                </button>
            </div>

            {{-- Home --}}
            <div>
                <span class="sidebar-label block ps-2.5 mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground-1 overflow-hidden whitespace-nowrap transition-all duration-300">Home</span>
            ...
            {{-- Management --}}
            <div>
                <span class="sidebar-label block ps-2.5 mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground-1 overflow-hidden whitespace-nowrap transition-all duration-300">Management</span>
                <ul class="space-y-0.5">
                    <li>
                        <a href="#"
                            ...
                            <span class="sidebar-label whitespace-nowrap overflow-hidden transition-all duration-300">Aspirasi</span>
                        </a>
                    </li>
                    <li>
                        <a href="#"
                            ...
                            <span class="sidebar-label whitespace-nowrap overflow-hidden transition-all duration-300">
                                Siswa
                                <span class="ms-1 text-[9px] bg-muted px-1.5 py-0.5 rounded-full font-semibold">Soon</span>
                            </span>
                        </a>
                    </li>
                    <li>
        ...
        {{-- Sidebar Footer --}}
        <div class="shrink-0 p-3 border-t border-sidebar-2-divider space-y-0.5 overflow-hidden">
            <a href="#"
                ...
                <span class="sidebar-label whitespace-nowrap overflow-hidden transition-all duration-300">What's new?</span>
            </a>
            <a href="#"
                class="sidebar-nav-item flex items-center gap-x-2.5 py-1.5 px-2.5 text-sm rounded-lg text-sidebar-2-nav-foreground hover:bg-sidebar-2-nav-hover transition-colors"
                title="Help & support">
                ...
                <span class="sidebar-label whitespace-nowrap overflow-hidden transition-all duration-300">Help & support</span>
            </a>
        </div>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-sidebar-2">
        ...
        <header class="shrink-0 flex items-center h-14 px-4 sm:px-5 gap-x-3">
            ...
            <button type="button" id="sidebar-toggle"
                ...
                <svg id="icon-collapse" class="size-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <rect width="18" height="18" x="3" y="3" rx="2"/>
                    <path d="M15 3v18"/>
                    <path d="m10 15-3-3 3-3"/>
                </svg>
                <svg id="icon-expand" class="size-4 hidden" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <rect width="18" height="18" x="3" y="3" rx="2"/>
                    <path d="M9 3v18"/>
                    <path d="m14 9 3 3-3 3"/>
                </svg>
                <svg id="icon-menu-mobile" class="size-4 hidden" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            {{-- Account Dropdown --}}
            <div class="hs-dropdown inline-flex [--strategy:absolute] [--auto-close:inside] [--placement:bottom-right] relative">
                <button id="hs-admin-dd" type="button"
                    ...
                    <div class="size-7 rounded-full bg-primary flex items-center justify-center text-primary-foreground text-xs font-bold">
                    ...
                    <svg class="hidden sm:block size-3.5 text-muted-foreground-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path d="m7 15 5 5 5-5"/><path d="m7 9 5-5 5 5"/>
                    </svg>
                </button>

                <div class="hs-dropdown-menu hs-dropdown-open:opacity-100 w-56 transition-[opacity,margin] duration opacity-0 hidden z-20 bg-dropdown border border-dropdown-line rounded-xl shadow-xl" role="menu">
                    <div class="py-2 px-3.5 border-b border-dropdown-divider">
                        <span class="font-semibold text-sm text-foreground">{{ Auth::guard('admin')->user()->username ?? 'Admin' }}</span>
        ...
        <main class="flex-1 overflow-hidden px-2 pb-2">
            <div class="h-full bg-background rounded-xl overflow-y-auto">
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    @if(session('success'))
                        <div class="p-3 rounded-lg bg-teal-500/10 border border-teal-500/20 text-teal-500 text-xs font-bold uppercase tracking-wider">
                            {{ session('success') }}
                        </div>
                    @endif

                    @if(session('error'))
                        <div class="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-bold uppercase tracking-wider">
                            {{ session('error') }}
                        </div>
                    @endif
